...
describe('parse_one_cmd', function()
  ...
  describe(':edit command family', function()
    itn('edit lit(abc) lit(def)', 'edit  abc  def')
    itn('edit lit({abc) lit(def)', 'e  {abc  def')
    itn('edit shell(echo abc).lit(def) lit(ghi)', 'e  `echo abc`def ghi')
    itn('edit expr("abc").lit(def) lit(ghi)', 'e  `="abc"`def ghi')
    itn('edit lit(`echo abc)', 'e  `echo\\ abc')
  end)
end)
