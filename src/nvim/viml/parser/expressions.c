  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  // TODO(ZyX-I): use ast_stack to determine and highlight printf format string
  ...
  const char *p = s + 1;
  size_t size = token.len - token.data.str.closed;
  kvec_withinit_t(StringShift, 16) shifts;
  kvi_init(shifts);
  if (token.type == kExprLexSingleQuotedString) {
